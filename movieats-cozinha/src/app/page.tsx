...
export default function KitchenLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Simulação de login da cozinha
      if (credentials.user === "cozinha" && credentials.password === "123456") {
         Swal.fire({
            icon: 'success',
            title: 'KDS Conectado',
            text: 'Acessando Painel de Produção',
            background: '#141414',
            color: '#fff',
            showConfirmButton: false,
            timer: 1500
         });
         router.push("/cozinha"); // Rota futura do KDS
      } else {
        throw new Error("Credenciais inválidas para a cozinha");
      }
    } catch (err: any) {
      Swal.fire({
        icon: 'error',
    ...
    <div className="flex flex-col min-h-screen p-6 md:p-12 items-center justify-center">
      <div className="w-full max-w-md flex flex-col justify-center py-8">
        ...
        >
          ...
          <h1 className="text-xl font-bold uppercase tracking-widest text-orange-600 mb-2">Painel da Cozinha - KDS</h1>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div className="space-y-5">
              <div className="flex flex-col space-y-2">
                <label className="text-[11px] font-black uppercase tracking-widest text-white/70 ml-1">Usuário KDS</label>
            ...
            <div className="pt-4">
              ...
              >
                {loading ? "Conectando..." : "ACESSAR KDS"}
        ...
        <div className="mt-12 text-center opacity-40">
          <p className="text-[10px] font-black text-white uppercase tracking-[0.4em]">© 2026 Movieats Business • KDS Module</p>
        </div>
      </div>
    </div>
  );
