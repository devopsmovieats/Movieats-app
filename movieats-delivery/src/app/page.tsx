...
import { User, Lock, ArrowRight, Flame, Truck } from "lucide-react";
...
export default function DeliveryLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Simulação de login do entregador
      if (credentials.user === "delivery" && credentials.password === "123456") {
         Swal.fire({
            icon: 'success',
            title: 'Entregador Conectado',
            text: 'Acessando Painel de Entregas',
            background: '#141414',
            color: '#fff',
            showConfirmButton: false,
            timer: 1500
         });
         router.push("/entregas"); // Rota futura do Delivery
      } else {
        throw new Error("Credenciais inválidas para o delivery");
      }
    } catch (err: any) {
      Swal.fire({
        icon: 'error',
    ...
    <div className="flex flex-col min-h-screen p-6 md:p-12 items-center justify-center">
      <div className="w-full max-w-md flex flex-col justify-center py-8">
        ...
        >
          ...
          <h1 className="text-xl font-bold uppercase tracking-widest text-orange-600 mb-2">Painel do Entregador</h1>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div className="space-y-5">
              <div className="flex flex-col space-y-2">
                <label className="text-[11px] font-black uppercase tracking-widest text-white/70 ml-1">Usuário Entregador</label>
              ...
              <div className="flex flex-col space-y-2">
                <label className="text-[11px] font-black uppercase tracking-widest text-white/70 ml-1">Senha</label>
                <div className="relative group">
                  <div className="absolute left-5 top-1/2 -translate-y-1/2 text-white/30 group-focus-within:text-orange-600 transition-colors">
                    <Lock size={20} />
                  </div>
            ...
            <div className="pt-4">
              ...
              >
                {loading ? "Conectando..." : "INICIAR ENTREGAS"}
        ...
        <div className="mt-12 text-center opacity-40">
          <p className="text-[10px] font-black text-white uppercase tracking-[0.4em]">© 2026 Movieats Business • Delivery Module</p>
        </div>
      </div>
    </div>
  );
