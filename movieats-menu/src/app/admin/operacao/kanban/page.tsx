...
interface Order {
  ...
  pago: boolean;
  tipo: "Mesa" | "Balcão" | "Entrega";
  mesa?: string;
  total: number;
}

const initialOrders: Order[] = [
  { id: "1024", clientName: "João Silva", itemsSummary: "2x Smash Duo, 1x Coca", status: "Pendente", createdAt: Date.now() - 1000 * 60 * 5, pago: true, tipo: "Mesa", mesa: "05", total: 85.30 },
  { id: "1025", clientName: "Maria Oliveira", itemsSummary: "1x Batata Rústica", status: "Preparo", createdAt: Date.now() - 1000 * 60 * 15, pago: false, tipo: "Balcão", total: 18.00 },
  { id: "1026", clientName: "Carlos Souza", itemsSummary: "3x Pizza Calabresa", status: "Entrega", createdAt: Date.now() - 1000 * 60 * 30, pago: true, tipo: "Entrega", total: 137.70 },
  { id: "1027", clientName: "Fernanda Costa", itemsSummary: "1x Suco Laranja", status: "Pendente", createdAt: Date.now() - 1000 * 60 * 2, pago: false, tipo: "Mesa", mesa: "02", total: 12.00 },
];
...
export default function KanbanPage() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        // Normalização de dados para garantir compatibilidade com fontes diferentes
        const normalized = parsed.map((o: any) => ({
          ...o,
          total: o.totalPedido || o.valorTotal || o.total || o.value || o.subtotal || o.price || 0,
          tipo: o.tipoPedido || o.tipo || o.origem || "Balcão",
          pago: o.pago !== undefined ? o.pago : false
        }));
    ...
    // Monitoramento do LocalStorage para reatividade entre abas
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === "movieats_orders" && e.newValue) {
  ...
  return (
    <DashboardLayout>
      <div className="h-[calc(100vh-140px)] w-full flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700 pl-6 pr-2">
        ...
        {/* Board */}
        <div className="flex-1 flex gap-4 overflow-x-auto pb-6 scrollbar-thin scrollbar-thumb-primary/20 scrollbar-track-transparent pr-4 mt-4">
          {COLUMNS.map((col) => (
            <div 
              key={col.id}
              onDragOver={handleDragOver}
              onDrop={(e) => handleDrop(e, col.id)}
              className="w-[320px] shrink-0 flex flex-col bg-black/20 border border-white/10 rounded-2xl overflow-hidden group/column h-full"
            >
              {/* Column Header */}
                ...
                {orders
                  ...
                  .map((order) => (
                    <div
                      ...
                      className="bg-[#141414] border border-white/10 p-3 rounded-xl shadow-lg cursor-grab active:cursor-grabbing hover:border-white/20 transition-all group relative overflow-hidden"
                    >
                      ...
                      <div className="flex flex-col gap-3">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            {/* Badge de Pagamento */}
                            <div className={`flex items-center gap-1 px-1.5 py-0.5 rounded-md border border-white/5 ${order.pago ? 'bg-emerald-500/10' : 'bg-red-500/10'}`}>
                              {order.pago ? (
                                <>
                                  <DollarSign className="w-2 h-2 text-emerald-500" />
                                  <CheckCircle2 className="w-2 h-2 text-emerald-500" />
                                  <span className="text-[7px] font-black text-emerald-500 uppercase tracking-tighter">Pago</span>
                                </>
                              ) : (
                                <>
                                  <DollarSign className="w-2 h-2 text-red-500" />
                                  <span className="text-[7px] font-black text-red-500 uppercase tracking-tighter">Pendente</span>
                                </>
                              )}
                            </div>
                        ...
                        {/* Origem e Valor (Redistribuído) */}
                        <div className="flex items-center justify-between mt-1 gap-2">
                          <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-lg border border-white/5 bg-white/[0.03]`}>
                            <span className="text-[8px] font-black text-white/70 uppercase tracking-widest whitespace-nowrap">
                               {order.tipo === "Mesa" ? `🏷️ Mesa ${order.mesa || '--'}` : 
                                order.tipo === "Entrega" ? "🛵 Entrega" : (order.tipo || "N/A")}
                            </span>
                          </div>
                          <div className="flex-1 border-b border-dashed border-white/5 mx-1 mb-1" />
                          <span className="text-[12px] font-black text-white tracking-tighter whitespace-nowrap">
                        ...
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="text-[10px] font-black text-white uppercase tracking-tight truncate">{order.clientName}</span>
                        </div>

                        <div className="pl-8 relative">
                          <div className="absolute left-3 top-0 bottom-0 w-[1px] bg-white/5" />
                          <p className="text-[9px] text-white/40 font-medium line-clamp-2 uppercase tracking-tighter leading-relaxed">
                            {order.itemsSummary}
                          </p>
                        </div>
                      </div>
