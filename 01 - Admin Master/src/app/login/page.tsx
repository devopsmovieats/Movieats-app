...
export default function LoginPage() {
  ...
  const handleRecoverPassword = async () => {
    const { value: userEmail } = await Modal.fire({
      ...
      inputPlaceholder: "nome@movieats.com.br",
  ...
  return (
    <div className="h-screen w-full flex items-center justify-center p-4 relative overflow-hidden font-body bg-[#0a0a0a]">
      ...
      <div className="w-full max-w-[340px] z-10 transition-all duration-700 animate-in fade-in slide-in-from-bottom-6 px-4 sm:px-0">
        <div className="bg-[#141414] border border-white/5 rounded-2xl p-7 shadow-2xl shadow-black/50">
          ...
          <div className="flex flex-col items-center mb-7">
            ...
            <span className="text-[8px] uppercase tracking-[0.3em] font-black text-muted-foreground">Painel Administrativo</span>
          </div>

          {/* Welcome Text */}
          <div className="mb-7 text-center sm:text-left cursor-default">
            <h2 className="font-headline text-base font-black text-white mb-1">Bem-vindo de volta</h2>
            <p className="text-[11px] text-muted-foreground font-medium">Equipe interna autorizada.</p>
          </div>

          {/* Form */}
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground ml-1" htmlFor="email">
                E-mail corporativo
              </label>
              <input 
                id="email"
                type="email"
                required
                placeholder="nome@movieats.com.br"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                className="w-full bg-white/[0.03] border border-white/5 rounded-lg px-4 py-2.5 text-sm text-white placeholder:text-muted-foreground/20 focus:border-primary/50 focus:ring-4 focus:ring-primary/10 transition-all outline-none"
              />
            </div>

            <div className="space-y-1.5">
              <label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground ml-1" htmlFor="password">
                Senha de acesso
              </label>
              <div className="relative group">
                <input 
                  ...
                  className="w-full bg-white/[0.03] border border-white/5 rounded-lg px-4 py-2.5 text-sm text-white placeholder:text-muted-foreground/20 focus:border-primary/50 focus:ring-4 focus:ring-primary/10 transition-all outline-none pr-12"
                />
                <button 
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 hover:bg-white/5 rounded-lg text-muted-foreground hover:text-white transition-all cursor-pointer"
                >
            ...
            <button 
              type="submit"
              disabled={isLoading}
              className="w-full bg-primary hover:bg-orange-500 active:scale-[0.98] disabled:opacity-70 disabled:active:scale-100 text-white font-black text-[10px] uppercase tracking-widest rounded-lg py-3.5 shadow-xl shadow-primary/20 transition-all flex items-center justify-center gap-2.5 group cursor-pointer"
            >
              {isLoading ? (
